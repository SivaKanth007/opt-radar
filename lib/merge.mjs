...
// Premium clock start (USCIS 30-business-day promise runs from here):
//  - premium from start → clock starts once biometrics are done
//  - upgraded later → clock starts at the upgrade (or biometrics, if bio came after)
//  → max(pp_upgrade_date, biometrics_date); fallback date_applied when neither
//    is reported yet. Validated on data: measured from this start, p50 = 26
//    business days and 94% of premium approvals land within 30 BD — measured
//    from date_applied the same cases look like "62 calendar days" (wrong baseline).
function ppStart(c) {
  if (!c.premium) return null;
  const cands = [c.pp_upgrade_date, c.biometrics_date].filter(Boolean);
  return cands.length ? cands.sort().at(-1) : (c.date_applied ?? null);
}
